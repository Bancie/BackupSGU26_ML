...
**Gợi ý khi thi:** Nếu đề nói _“xấp xỉ nếu cần”_, bạn có thể nội suy (20,5; 35) **hoặc** lấy nguyên (20; 35) — quan trọng là trình tự **công bố công thức** cho điểm thẳng.
...
(Trong boxplot, trục tung là thang tuổi: vẽ hộp từ Q1 đến Q3, đường giữa hộp là median, “râu” xuống min và lên max **theo quy ước môn học** — thường **giới hạn bởi** $1{,}5\cdot\mathrm{IQR}$; giá trị nằm ngoài bằng **điểm** hoặc “râu kéo đến giá trị hợp lệ lớn nhất trong khoảng”).

#### Bước 2 — Rào 1,5·IQR (dùng IQR đã tính)

- Với **IQR = 15** (Q1=20, Q3=35):  
  Cận trên: $35 + 1{,}5 \times 15 = 35 + 22{,}5 = 57{,}5.$  
  Vì **70 > 57,5** → **70 thường xem là ngoại lai (outlier)** phía trên; trên hình có thể vẽ 70 bằng _chấm_ riêng, râu dừng tại 52 (hoặc theo cách bài tập/WeMooc của lớp).
...
| ----------------------------------------------------------------------- | ------------- |
| Lớn nhất: $(12{,}99\ldots,\, 27{,}25]$ tương ứng khoảng từ 13 lên tới 27,25 | **14**        |
| Kế tiếp: đến 41,5                                                       | **9**         |
|                                                                         | **3**         |
...
**Ánh xạ tuyến tính/afin từ V** (kiểm tra từng cột, $v \in \{1,2,3,4,5\}$):
...
Nếu giảng viên chấm theo cách _chỉ_ xem V là mã hạng (không tính hiệu), có thể giải thích **(2) thứ tự**; khi đó cùng thứ tự bậc tăng ở dưới sẽ **rộng** hơn (gồm cả W₄) — ghi ở mục “Ghi chú bài tập thứ tự”.
...
- **$\mathrm{TP}_k$** = ô $(k,k)$.
- **$\mathrm{FN}_k$** = tổng hàng $k$ trừ $\mathrm{TP}_k$.
- **$\mathrm{FP}_k$** = tổng cột $k$ trừ $\mathrm{TP}_k$.
...
- **A:** TP=2, FP=1, FN=2 → $P=2/3$, $R=2/4=0{,}5$, $\text{F1}\approx 0{,}571$.
- **B:** TP=2, FP=1, FN=1 → $P=2/3$, $R=2/3$, $\text{F1}\approx 0{,}667$.
- **C:** TP=2, FP=2, FN=1 → $P=0{,}5$, $R=2/3$, $\text{F1}\approx 0{,}571$.
...
### Câu 3.3 — 14 mẫu, decision list, positive = _play_ = _yes_
...
4. **Ngược lại** (thường _rainy_ và _không_ gió) → **play = yes**

_Lưu ý: dòng_ **overcast** _luôn đi tới rule 1, không cần xem gió. Dòng_ **sunny** _và không gió → rule 3 nói_ **no** _(mẫu 9, 8 khớp_ **no** _như thực, mẫu 9 thực_ **yes** _→ sai theo bộ quy tắc này). Dòng_ **sunny** _+ gió_ **TRUE** _→ rule 2_ **no** _(mẫu 11: thực_ **yes** _→ sai nếu nộp đúng quy tắc trên bài)._

#### Cách làm bài: bảng 14 dòng

Với từng mẫu, viết: outlook, windy, rồi thử 1 → 2 → 3 → 4, ghi dựa đoán, so với cột _play_ thật. Sau cùng điền ma trận 2×2 (thực yes/no × dự yes/no, positive = yes).
...
| Misclass. rate               | $(FP+FN)/N$                      | 15/165 = **1/11** ≈ 0,0909   |
...
Ghi chú: $\mathrm{Sensitivity}=\mathrm{Recall}=\mathrm{TPR}$; $\mathrm{Specificity}=\mathrm{TNR}$.

---

_Issue gốc: [PER-4375](https://linear.app/bancie/issue/PER-4375/checklist-on-phan-3)._
