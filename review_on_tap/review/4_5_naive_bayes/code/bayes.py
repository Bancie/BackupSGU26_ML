"""Naive Bayes: categorical + Gaussian hybrid (tham số hoá) + multinomial — PER-4810."""
    ...
    def fit(self, rows: list[dict[str, Any]], feature_cols: list[str], target: str) -> None:
        ...
        for f in feature_cols:
            ...
            for c in self.classes_:
                ...
                for v in sub:
                    cnt[v] += 1
                denom = len(sub) + self.alpha * v_size
...
class GaussianHybridNB:
    """
    Thuộc tính rời rạc: Laplace.
    Thuộc tính số: Gaussian, μ và độ lệch chuẩn mẫu (ddof=1).
    Mặc định giống bài tennis số (Outlook, Windy / Temperature, Humidity).
    """
    ...
    def __init__(
        ...
        self.alpha = float(alpha)
        self.cat_cols: list[str] = list(categorical_features or ["Outlook", "Windy"])
        self.num_cols: list[str] = list(numeric_features or ["Temperature", "Humidity"])
