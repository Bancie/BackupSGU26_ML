#!/usr/bin/env python3
"""PER-4783 — đối chiếu đáp án. Chạy: python verify_exercises.py (trong thư mục code)."""
...
def _print_q1() -> None:
    print("=" * 70)
    print("CÂU 1 — Tennis (thuộc tính rời rạc), Laplace α = 1")
    target = "Play"
    feat_cols = q1_split()
    alpha = 1.0
...
def _print_q2() -> None:
    print("\n" + "=" * 70)
    print("CÂU 2 — Tennis (nhiệt độ & độ ẩm số), Gaussian + Laplace rời rạc, α = 1")
    tgt = "Play"
    alpha = 1.0
    norm_rows = []
    for r in Q2_ROWS:
    ...
    for c in g.classes_:
        print(f"\n  Lớp Play = {c}:")
        for f in g.cat_cols:
            for v, lv in sorted(g._cat.cond_log_prob[c][f].items(), key=lambda x: str(x[0])):
                print(f"    P({f}={v!r} | {c}) = {math.exp(lv):.6f}")
...
def _print_q3() -> None:
    print("\n" + "=" * 70)
    print("CÂU 3 — Email spam, Multinomial NB, V = 7, Laplace α = 1")
    alpha = 1.0
