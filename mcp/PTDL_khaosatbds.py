"""MCP server: FAISS + sentence-transformers search over PTDL khảo sát BĐS (parsed markdown)."""
...
# Repo has a folder named `mcp/`; when running this script, that directory can be first on sys.path
# and shadow the PyPI package `mcp`. Drop script dir entries so MCP SDK resolves correctly.
...
_DEFAULT_CORPUS = _REPO_ROOT / "parsing" / "PTDL_khaosatbds"
...
def _corpus_dir() -> Path:
    raw = os.environ.get("PTDL_CORPUS_DIR", "").strip()
...
def build_or_load() -> IndexStore:
    ...
    src = _source_path()
    if not src.is_file():
        raise FileNotFoundError(f"PTDL corpus markdown not found: {src}")

    model_name = (
        os.environ.get("PTDL_EMBED_MODEL", "").strip()
        or os.environ.get("EDA_EMBED_MODEL", "").strip()
        or _DEFAULT_MODEL
    )
    cdir = _cache_dir()
    ...
    if _validate_cache(meta_path, cdir, src, model_name):
        ...
        _log(f"[ptdl-bds-mcp] Loaded FAISS cache: {len(chunks)} chunks, dim={dim}")
        return IndexStore(dim, model_name, src, chunks, index)

    cdir.mkdir(parents=True, exist_ok=True)
    _log("[ptdl-bds-mcp] Building embeddings (first run may download the model)...")
    ...
    _log(f"[ptdl-bds-mcp] Built index with {len(records)} chunks, dim={dim}")
...
mcp = FastMCP("ptdl-khaosatbds-faiss")


@mcp.tool()
def ptdl_search(query: str, top_k: int = 8) -> str:
    """Semantic search over the PTDL khảo sát BĐS parsed corpus (parsing.md).

    Args:
        query: Question or keywords (Vietnamese or other languages supported by the embedding model).
...
@mcp.tool()
def ptdl_chunk(chunk_id: int) -> str:
    """Fetch full markdown text for a chunk id (from ptdl_search results).

    Args:
        chunk_id: Index returned by ptdl_search in the chunk_id field.
    """
    store = _store()
...
@mcp.tool()
def ptdl_stats() -> str:
    """Summary of the PTDL BĐS corpus: paths, embedding model, and chunk counts."""
    store = _store()
    summary = {
